...
contract TokenSale {
    ...
    event Airdrops(address indexed reciever, uint256 amount);

    event Deposit(address indexed sender, uint256 amount, uint256 balance);

    event NewAddr(address addr);

    event Transfer(address _address, uint256 _amount, uint256 _balance);
    ...
    function getAirdrop() public {
        require(
            !airdropWallets[msg.sender],
            "You have already received airdrop"
        );

        //Require that the contract has enough tokens
        require(
            tokenContract.balanceOf(address(this)) >= airdropAmount,
            "insufficient token"
        );
    ...
    function depositBNB(
        ...
        require(
            ...
            "Unable to transfer token to buyer"
        );

        if (_referralAddress != address(0) && _referralAddress != msg.sender) {
            require(
                tokenContract.transfer(_referralAddress, referralAmount),
                "Unable to transfer token to referral"
            );
